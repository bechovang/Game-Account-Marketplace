...
/**
 * Validation schema for account update form
 */
...
/**
 * EditListingPage - Form for editing existing account listings
 * Pre-fills form with existing account data
...
const EditListingPage = () => {
  ...
  useEffect(() => {
    if (account && !isLoaded) {
      ...
      setValue('level', account.level || undefined);
      setValue('rank', account.rank || '');
      setValue('price', account.price);
      setValue('images', account.images || []);
      setImagePreviews(account.images || []);
      setIsLoaded(true);
    }
  }, [account, setValue, isLoaded]);
  ...
  const onSubmit = async (data: UpdateAccountInput) => {
    ...
    try {
      // For now, use placeholder URLs for images if no images
      const input: UpdateAccountInput = {
        ...data,
        images: data.images.length > 0 ? data.images : ['https://placeholder.example.com/default.jpg'],
      };

      const result = await updateAccount({ variables: { id, input } });
      ...
      toast.error('Failed to update account listing');
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Edit Account Listing</h1>
        <p className="text-gray-600 mt-1">Update your game account listing</p>
        <p className="text-sm text-gray-500 mt-1">Listing ID: {id}</p>
      </div>
